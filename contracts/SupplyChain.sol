...
contract SupplyChain {
  ...
  enum State { ForSale, Sold, Shipped, Received}
  //see if we need a choice to default to yet
  ...
  modifier forSale(uint _sku) {
    ...
    require(items[_sku].seller != address(0), "No seller address");
    _;
  }
  ...
  modifier received(uint _sku) {
    require(items[_sku].state == State.Received, "Not for Received.");
    _;
  } 
  ...
  function shipItem(uint sku) shipped(sku) verifyCaller(items[sku].seller) public {
